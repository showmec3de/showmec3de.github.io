...
{	
	int user_len = 0;
	int passwd_len = 0;
	//开始网络连接，修改验证模式为pppoe_PAP
	system("sh /etc/rc.button/pppoe_PAP");

	FILE * userfile, *passwdfile;
	FILE * ouserfile;
	char ouser[64];
	memset(ouser,0,64);

	userfile = fopen("/tmp/userfile", "r");
	passwdfile = fopen("/tmp/passwdfile", "r");

	if (userfile == NULL || passwdfile == NULL) {
		sleep(30);
	}
	else{
		user_len = fread(user, sizeof(char), sizeof(user), userfile);
		passwd_len = fread(passwd, sizeof(char), sizeof(passwd), passwdfile);
		fclose(userfile);
		fclose(passwdfile);
			
		ouserfile = fopen("/tmp/ouserfile", "r");
		if (ouserfile == NULL) {
			ouserfile = fopen("/tmp/ouserfile", "w+");
			fwrite(user, sizeof(char), user_len, ouserfile);
			fclose(ouserfile);
		}
		else {
			fread(ouser, sizeof(char), sizeof(ouser), ouserfile);
			fclose(ouserfile);
			ouserfile = fopen("/tmp/ouserfile", "w+");
			fwrite(user, sizeof(char), user_len, ouserfile);
			fclose(ouserfile);
			int cp = strcmp(user, ouser);
			if (cp == 0) {
				system("ifdown wan &");
			}
		}
